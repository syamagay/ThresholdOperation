...
void ReadDatFile::init(){
  ...
  under_flow=0;
  vector_iterator=0;
  value_map=std::vector<std::vector<std::vector<double>>>(4, std::vector<std::vector<double>>(RD53a_column,std::vector<double>(RD53a_row)));
  ...
  via_parameter1=std::vector<std::vector<int>>(RD53a_column, std::vector<int>(RD53a_row));
  for(int it=0;it<4;it++){
    for(int cl=0;cl<RD53a_column;cl++){
      for(int ro=0;ro<RD53a_row;ro++){
	value_map[it][cl][ro]=0;
      }
    }
  }
...
int ReadDatFile::read_datfile(){
  ...
  if(fin.fail()){
    std::cout << dat_file << " : akaaaaaann" << std::endl;
    return 1;
  }
  if(getline(fin,plotType)){
	...
	for(int i=0;i<RD53a_column;i++){    
	  fin >> value_map[vector_iterator][i][j];
	}
      }
    }
  }
...
void ReadDatFile::set_parameter(std::string name1,int par1,std::string name2,int par2,std::string name3,int par3,std::string name4,double par4[400][192],std::string name5,double par5[400][192],std::string name6,double par6[400][192],std::string via_name1,double viapar1[400][192]){
  parName1=name1;
  parameter1=par1;
  parName2=name2;
  parameter2=par2;
...
void ReadDatFile::make_TTree(std::string output_dir,int count,int target_threshold,int scan_time){
  ...
  tree->Branch("Threshold_via",&Threshold_via,"Threshold_via/D");
  tree->Branch("Noise_via",&Noise_via,"Noise_via/D");
  ...
  tree->Branch(via_parName1.c_str(),&TDAC_via,Form("%s/I",via_parName1.c_str()));
  ...
  for(Column=0;Column<RD53a_column;Column++){
    for(Row=0;Row<RD53a_row;Row++){
      Threshold=value_map[0][Column][Row];
      Noise=value_map[1][Column][Row];
      Threshold_via=value_map[2][Column][Row];
      Noise_via=value_map[3][Column][Row];

      TDAC=parameter4[Column][Row];
      Enable=parameter5[Column][Row];
      InjEn=parameter6[Column][Row];
      TDAC_via=via_parameter1[Column][Row];
...
int read_dat_via(std::string target_dir,std::string target_dir2,std::string output_dir, int count, int target_threshold=0, int scantime=0){
  ...
  std::string via_dat_file=target_dir2+"/"+dat;
  std::string via_dat_file2=target_dir2+"/"+dat2;
  ...
  std::string via_json=target_dir2+json;
  ...
  ifstream jf_via(via_json);
  if(jf_via.fail()){
    std::cout << via_json << std::endl;
    std::cout << "initial json ga akaaaaaann" << std::endl;
    return 1;
  }  
  ...
  i=0;
  picojson::value val_via;
  jf_via >> val_via;
  std::string viaparName1="TDAC_via";
  double viapar1[400][192];
  picojson::array& array_via=val_via.get<picojson::object>()
    ["RD53A"].get<picojson::object>()
    ["PixelConfig"].get<picojson::array>();
  for (picojson::array::iterator it = array_via.begin(); it != array_via.end(); it++) {
    picojson::array& TDAC_via_List=it->get<picojson::object>()
      ["TDAC"].get<picojson::array>();
    j=0;
    for (picojson::array::iterator it2 = TDAC_via_List.begin(); it2 != TDAC_via_List.end(); it2++){
      viapar1[i][j]=it2->get<double>();
      //std::cout << viapar1[i][j] << std::endl;
      j++;
    }
    i++;
  }
  ...
  ReadDatFile*rd=new ReadDatFile(dat_file);
  rd->set_parameter(parName1,par1,parName2,par2,parName3,par3,parName4,par4,parName5,par5,parName6,par6,viaparName1,viapar1);
  rd->set_scanpar(scanparName1,scanpar1,scanparName2,scanpar2,scanparName3,scanpar3,scanparName4,scanpar4);
  
  int read_dat=rd->read_datfile();
  rd->add_datfile(dat_file2);
  int read_dat2=rd->read_datfile();
  rd->add_datfile(via_dat_file);
  int read_via_dat=rd->read_datfile();
  rd->add_datfile(via_dat_file2);
  int read_via_dat2=rd->read_datfile();

  if(read_dat==0&&read_dat2==0&&read_via_dat==0&&read_via_dat2==0){
    std::cout << "Reading dat file is succeed!" << std::endl;
