...
i_candidate=(85)
#i_candidate=(0 25 50 75)
#i_candidate=(90 91 92 93 94 95 96 97 98 99)
...
#    target_threshold=$target_threshold_start
    ...
    for middle in ${middle_candidate[@]}
    do
	diff=$((middle-target_threshold))
	diff=$(echo "sqrt(${diff}^2)" | bc)
	#echo "diff : $diff"
	if [ $diff -le $diff_min ];then
	    #echo "diff_min : $diff_min"
	    diff_min=$diff
	    target_threshold_middle=$middle
	elif [ $diff -eq $diff_min ] && [ $middle -gt $target_threshold_middle ];then
	    target_threshold_middle=$middle
	fi
    done
    ...
    cd $YARR_DIR
    ./scripts/doScan.sh -a rd53a -m ${module_name} -s diff_tune_globalthreshold_selected -t $target_threshold_middle > ${output_dir}/gayaLog${i}.log 2>&1
    ./scripts/doScan.sh -a rd53a -m ${module_name} -s diff_tune_pixelthreshold_selected -t $target_threshold_middle >> ${output_dir}/gayaLog${i}.log 2>&1
    ./scripts/doScan.sh -a rd53a -m ${module_name} -s diff_thresholdscan -t $target_threshold_middle >> ${output_dir}/gayaLog${i}.log 2>&1
    ...
    cd $YARR_DIR
    ./scripts/doScan.sh -a rd53a -m ${module_name} -s diff_retune_globalthreshold_selected -t $target_threshold >> ${output_dir}/gayaLog${i}.log 2>&1
    ./scripts/doScan.sh -a rd53a -m ${module_name} -s diff_retune_pixelthreshold_selected -t $target_threshold >> ${output_dir}/gayaLog${i}.log 2>&1

    ./scripts/doScan.sh -a rd53a -m ${module_name} -s diff_analogscan -t $target_threshold >> ${output_dir}/gayaLog${i}.log 2>&1
    ./scripts/doScan.sh -a rd53a -m ${module_name} -s diff_thresholdscan >> ${output_dir}/gayaLog${i}.log 2>&1
    
    cd $TMP_DIR
    root -l -q read_dat_via.cpp+'("'$data_dir'","'$via_path'","'$output_dir'",'$i','$target_threshold')'
    
    cd $YARR_DIR
    ./scripts/doScan.sh -a rd53a -m ${module_name} -s diff_noisescan
done
