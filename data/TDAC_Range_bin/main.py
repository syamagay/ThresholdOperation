...
def Loop(filename):
    ...
    for i_event in range(0, total_entry):
        ...
        if tree.TDAC == 15 or tree.TDAC == -15 :
            ...
            if not tree.target_threshold in target_threshold_list :
                i_target_threshold=len(target_threshold_list)
                target_threshold_list.append(tree.target_threshold)
                Threshold_TDAC15.append([])
                Threshold_TDACm15.append([])
            else:
                i=0
                for tt in target_threshold_list :
                    if tt == tree.target_threshold :
        ...
        #if tree.Column==299 and tree.Row > 50 and tree.Row < 60 :
        #if tree.Column==267 and tree.Row > 10 and tree.Row < 20 :
        if tree.Column==267 and ( tree.Row == 8 or tree.Row == 24 or tree.Row == 42 or tree.Row == 45 or tree.Row == 61 or tree.Row == 74 or tree.Row == 155 or tree.Row == 160 ) :
            for Analysis in AnalysisList:
                Analysis.SetEvent(tree.Column, tree.Row, tree.target_threshold, tree.Threshold, tree.Noise, tree.TDAC,tree.DiffVth1)
